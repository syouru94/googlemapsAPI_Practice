...
geocode_result = gmaps.geocode("中正區")
geocode_result
#建立一個包含台北市行政區的list
...
for district in districts:
    ...
    print("以"+district+"為中心半徑2500公尺的拉麵店家數量: "+str(len(gmaps.places_radar(keyword="拉麵",
                                                                      location=loc, radius=2500)['results'])))
...
output
#水平合併所需要的資訊
df_1 = output.loc[:, "formatted_address":"formatted_phone_number"]
df_2 = output.loc[:, "international_phone_number":"name"]
df_3= output.loc[:,"rating"]
...
fixed_df=rating_df[["formatted_address","name","international_phone_number","rating"]]
